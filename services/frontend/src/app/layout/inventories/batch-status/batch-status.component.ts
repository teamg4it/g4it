...
export class BatchStatusComponent implements OnInit {
    ...
    batchStatusCode = input("");
    type = input("loading");
    cssClass: string = "";
    toolTip: string = "";
    betweenDiv: string = "";
    createTime = input<Date | undefined>(undefined);
    batchLoading = input(false);
    inventoryId = input(0);
    taskId = input("");
    localCreateTime = computed(() => new Date(this.createTime()?.toString() + "Z"));
    ...
    getBatchStatusUpdates(): void {
        const defaultClasses =
            "text-white text-lg border-circle p-1-5 w-2rem h-2rem text-center";

        if (
            Constants.EVALUATION_BATCH_RUNNING_STATUSES.includes(this.batchStatusCode())
        ) {
