...
public class RenewService {
    ...
    public RenewRest getRenewDetailsDigitalService(Long workspace, String digitalServiceUid) {
        Workspace workspaceEntity = workspaceRepository.findById(workspace).orElseThrow(() ->
                new G4itRestException("400", String.format(ErrorConstants.WORKSPACE_NOT_FOUND, workspace)));
        final LocalDateTime now = LocalDateTime.now();
        final Integer retentionDay = Optional.ofNullable(workspaceEntity.getDataRetentionDay())
                .orElse(Optional.ofNullable(workspaceEntity.getOrganization().getDataRetentionDay())
                        .orElse(dataRetentiondDay));
        DigitalService digitalService = digitalServiceRepository.findByWorkspaceAndUid(workspaceEntity, digitalServiceUid).orElseThrow(() ->
                new G4itRestException("400", String.format(ErrorConstants.DIGITAL_SERVICE_NOT_FOUND, digitalServiceUid)));
        long daysSinceLastUpdate = java.time.Duration.between(
    ...
    public RenewResponseRest renewDigitalService(Long workspace, String digitalServiceUid,RenewUpdateRest renewUpdateRest) {
