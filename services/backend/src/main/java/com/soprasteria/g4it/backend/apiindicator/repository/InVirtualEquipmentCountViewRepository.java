...
@Repository
public interface InVirtualEquipmentCountViewRepository
        extends JpaRepository<InVirtualEquipmentCountView, Long> {

    @Query(
            name = "InVirtualEquipmentCountView.findVirtualEquipmentCountIndicatorsByInventoryId",
            nativeQuery = true
    )
    List<InVirtualEquipmentCountView> findVirtualEquipmentCountIndicators(@Param("inventoryId") Long inventoryId);
}
